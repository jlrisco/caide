...
class SingleFarm(Coupled):
    """Clase que representa la granja de sensores de Oahu."""

    def __init__(self, parent_folder: str='data', sensors_activate: datetime=datetime.datetime.now(), sensors_passivate: datetime=datetime.datetime.now()):
        """Función de inicialización."""
        super().__init__("DataCenter")
        self.base_folder = os.join(parent_folder, self.name)
        self.sensors_activate = sensors_activate
        self.sensors_passivate = sensors_passivate  
        
        # OAHU
        # Sensors' basic information:
        ...
        # TODO: This should be computed at simulation time eventually:
        ...
        # Main fog server:
        farm = FarmServer("Oahu", sensor_names, sensor_latitudes, sensor_longitudes, sensor_means, sensor_stdevs)
        self.add_component(farm)
        # Sensors atomic models:
        for sensor_name in sensor_names:
            sensor = VirtualNode(sensor_name, self.base_folder, sensors_activate, sensors_passivate)
            db = DevsCsvFile(sensor_name, ['source', 'timestamp', 'radiation'], self.base_folder.replace('input', 'output'))
...
class MainTest(Coupled):
    ...
    def __init__(self, name: str, commands_path: str, sensors_folder: str = os.path.join('data', 'input', 'sensors_data')):
        ...
        for sensor_name in sensor_names:
            sensor = VirtualNode(name=sensor_name, parent_folder=os.path.join('data','input',self.name, farm_name))
...
if __name__ == "__main__":
    ...
    coupled = SingleFarm(model_name, os.path.join('data', 'input', 'simulations', 'main_test.txt'))
